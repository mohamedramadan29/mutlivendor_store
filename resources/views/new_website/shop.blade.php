            ...
            <div class="full-width_border border-2" style="border-color: #eeeeee;">
                <div class="shop-banner position-relative ">
                    ...
                    <div class="shop-banner__content container position-absolute start-50 top-50">
                        <h2 class="stroke-text h1 smooth-16 text-uppercase fw-bold mb-3 mb-xl-4 mb-xl-5 text-center"> المتجر  </h2>
                    ...
                    <div
                        ...
                        <form class="filter-choice select-form" name="sortProducts" id="sortProducts">
                            ...
                            <select aria-label="Sort Items" name="sort" title="sort-by"
                                ...
                                <option @if(isset($_GET['sort']) && $_GET['sort'] == 'price_from_hieght_low') selected
                ...
                <nav aria-label="Page navigation">
                    <ul class="pagination justify-content-center">
                        <li class="page-item @if ($products->onFirstPage()) disabled @endif">
                            <a class="page-link"
                               href="{{ $products->previousPageUrl() . (request()->has('sort') ? '&sort=' . request()->sort : '') }}"
                               aria-label="Previous">
                        ...
                        @for ($i = $start; $i <= $end; $i++)
                            <li class="page-item @if ($i === $currentPage) active @endif">
                                <a class="page-link"
                                   href="{{ $products->url($i) . (request()->has('sort') ? '&sort=' . request()->sort : '') }}">{{ $i }}</a>
                            </li>
                        @endfor

                        <li class="page-item @if (!$products->hasMorePages()) disabled @endif">
                            <a class="page-link"
                               href="{{ $products->nextPageUrl() . (request()->has('sort') ? '&sort=' . request()->sort : '') }}"
                               aria-label="Next">
